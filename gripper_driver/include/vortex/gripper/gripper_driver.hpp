...
class GripperDriver {
   public:
    GripperDriver(boost::asio::io_context& io,
                  std::string port,
    ...
    SerialStatus init_serial();

    std::uint16_t joy_to_pwm(double joy_value);

    SerialStatus send_pwm(uint16_t pwm_shoulder,
                          uint16_t pwm_wrist,
                          uint16_t pwm_claw);
    SerialStatus stop_gripper();
    SerialStatus start_gripper();
   ...
   private:
    ...
    std::function<void(const std::vector<double>&, SerialStatus)>
        encoder_callback_;

    SerialStatus send_packet(std::uint16_t id,
    ...
    bool try_extract_packet(std::uint16_t& id,
                            std::vector<std::uint8_t>& payload);

    static std::uint8_t checksum_xor(std::uint16_t id,
                                     const std::uint8_t* data,
                                     std::size_t len);
