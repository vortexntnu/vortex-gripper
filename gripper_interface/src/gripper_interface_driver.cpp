...
GripperInterfaceDriver::GripperInterfaceDriver(short i2c_bus,
    ...
    bus_fd_ = open(i2c_filename.c_str(), O_RDWR);  // Open the I2C bus for reading and writing (O_RDWR)
    if (bus_fd_ < 0) {
        throw std::runtime_error(std::format("ERROR: Failed to open I2C bus {} : {}", i2c_bus_, strerror(errno)));
    }
}
...
void GripperInterfaceDriver::send_pwm(const std::vector<std::uint16_t>& pwm_values) {
    try {
        constexpr std::size_t i2c_data_size = 1 + 3 * 2;  // 3 thrusters * (1xMSB + 1xLSB)
        ...
        std::ranges::for_each(pwm_values, [&](std::uint16_t pwm) {
            ...
            std::copy(bytes.begin(), bytes.end(), std::back_inserter(i2c_data_array));
        });

        if (ioctl(bus_fd_, I2C_SLAVE, i2c_address_) < 0) {
            throw std::runtime_error(std::format("Failed to open I2C bus {} : {}", i2c_bus_, strerror(errno)));
            return;
        }

        if (write(bus_fd_, i2c_data_array.data(), i2c_data_size) != i2c_data_size) {
            throw std::runtime_error(std::format("Error: Failed to write to I2C device : {}", strerror(errno)));
        }
...
void GripperInterfaceDriver::stop_gripper() {
    try {
        ...
        spdlog::error("ERROR: Failed to send stop gripper command - {}", e.what());
    } catch (...) {
        spdlog::error("ERROR: Failed to send stop gripper command - unknown error");
    }
}
...
void GripperInterfaceDriver::start_gripper() {
    try {
        ...
        spdlog::error("ERROR: Failed to send start gripper command - {}", e.what());
    } catch (...) {
        spdlog::error("ERROR: Failed to send start gripper command - unknown error");
    }
}
...
std::vector<double> GripperInterfaceDriver::encoder_read() {
    try {
        ...
        if (read(bus_fd_, i2c_data_array.data(), i2c_data_size) != static_cast<ssize_t>(i2c_data_size)) {
            throw std::runtime_error(std::format("Error: Failed to read from I2C device : {}", strerror(errno)));
        }
        ...
        std::ranges::transform(
            ...
            [&](std::size_t idx) -> double {
                std::array<std::uint8_t, 2> pair = { i2c_data_array[2 * idx], i2c_data_array[2 * idx + 1] };
